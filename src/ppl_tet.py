...
def createGripperTranslation(direction_vector,desired_distance=0.15,min_distance=0.1):
    ...
    gripper_trans.direction.vector.x = direction_vector.x   #### check this out
    gripper_trans.direction.vector.y = direction_vector.y
    gripper_trans.direction.vector.z = direction_vector.z
    gripper_trans.direction.header.frame_id = "base_footprint"
...
def add_offset_om_arm(grasp_pose):
    ...
    print("header:", grasp_pose.header)
...
def createGrasp(grasp_pose, allowed_touch_objects=[], pre_grasp_posture=None, grasp_posture=None, pre_grasp_approach=None, post_grasp_retreat=None, id_grasp="pick_place"):  #id_grasp = "grasp_"
...
def generate_grasps(pose, width=0.1):
    '''Send a request to the Grasp generator service'''
    print 'generate grasps'
...
if __name__=='__main__':
    ...
    scene.add_box("table", p, (0.1, 0.5, 0.1))
    ...
    goal = createPickupGoal("arm", "part", pose_grasp)  ## possible grasps removed by me
